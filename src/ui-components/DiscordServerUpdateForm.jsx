...
export default function DiscordServerUpdateForm(props) {
  const {
    id: idProp,
    discordServer: discordServerModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = discordServerRecord
      ? { ...initialValues, ...discordServerRecord }
      : initialValues;
    setHasColonyNotificationBotAdded(cleanValues.hasColonyNotificationBotAdded);
    setIdDiscord(cleanValues.idDiscord);
    setName(cleanValues.name);
    setErrors({});
  };
  const [discordServerRecord, setDiscordServerRecord] = React.useState(
    discordServerModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(DiscordServer, idProp)
        : discordServerModelProp;
      setDiscordServerRecord(record);
    };
    queryData();
  }, [idProp, discordServerModelProp]);
  React.useEffect(resetStateValues, [discordServerRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            DiscordServer.copyOf(discordServerRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DiscordServerUpdateForm")}
      {...rest}
    >
      <SwitchField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || discordServerModelProp)}
